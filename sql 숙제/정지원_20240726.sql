...
-- 18. 입사일로부터 오늘날짜까지 몇일이 지났는지 일로 출력하세뇨.
--  입사일, 이름, 성, 직무도 같이 출력
...
select next_day(sysdate,'금')-sysdate from hr.employees;

-- 25. 이름 , 성, 직무, 입사일을 출력할 때 입사일로부터 다음 목요일이 언제였는지 출력하세요.
select first_name, last_name, job_id, hire_date , next_day(hire_date,'목') from hr.employees;
-- 26 오늘부터 5개월후는 몇일입니까?
...
select * from hr.employees where next_day(hire_date, '목') = '01/01/18';
...
-- 32 각 상원이 직무를 담당한 달은 몇달인지 출력하시오.
select first_name, last_name, job_id, hire_date, trunc(months_between(sysdate,hire_date)) from hr.employees;
...
-- 출력되게 하시오
select '나는'|| ' '||upper(last_name)|| ' ' || lower(last_name) ||' ' || '입니다' from hr.employees;
select concat('나는',concat(upper(last_name), concat(' ' , concat(lower(last_name),'입니다')))) from hr.employees;
-- 37. 성과 이름을 붙여 출력
...
--- 46. o가 있는 성중 o가 몇번째에 있는 위치인지 출력하시오. 
--      직원번호 성, 성의 위치, 직무
